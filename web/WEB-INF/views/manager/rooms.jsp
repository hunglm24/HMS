...
<main class="page-container room-management-page">
    <section class="room-management-hero panel">
        <div class="room-management-hero__copy">
            ...
            <p>Quản lý loại phòng và phòng vật lý trong cùng một màn, nhưng tách riêng theo từng tab.</p>
        </div>

        <div class="room-management-hero__actions">
            <button class="btn" type="button" data-room-mgmt-open="room-type"
                    onclick="window.RoomManagement && window.RoomManagement.openRoomTypeModal();">
                + Thêm loại phòng
            </button>
            <button class="btn btn-secondary" type="button" data-room-mgmt-open="room"
                    onclick="window.RoomManagement && window.RoomManagement.openRoomModal();">
                + Thêm phòng
            </button>
        </div>
    </section>
    ...
    <nav class="room-management-tabs tabs" aria-label="Các tab quản lý phòng">
        <a class="tab ${activeTab eq 'room-types' ? 'active' : ''}"
           href="${pageContext.request.contextPath}/manager/rooms?tab=room-types">Loại phòng</a>
        <a class="tab ${activeTab eq 'rooms' ? 'active' : ''}"
           href="${pageContext.request.contextPath}/manager/rooms?tab=rooms">Phòng</a>
    </nav>
    ...
    <section class="room-management-content">
        <c:choose>
            <c:when test="${activeTab eq 'rooms'}">
                <jsp:include page="/WEB-INF/views/manager/fragments/rooms-tab.jsp" />
            </c:when>
            <c:otherwise>
                <jsp:include page="/WEB-INF/views/manager/fragments/room-types-tab.jsp" />
            </c:otherwise>
        </c:choose>
    </section>
</main>
